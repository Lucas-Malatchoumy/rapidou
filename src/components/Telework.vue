...
export default {
  ...
  async mounted() {
    axios
      .get("https://reeplay.online/location")
      .then((response) => {
  ...
  methods: {
    register() {
      ...
      axios
      .put(`https://reeplay.online/location/${mode}`, this.form)
      .then((response) => {
        console.log(response.data);
        // this.locations = response.data
        this.display()
    ...
    try {
      // console.log(this.form);
      const response = await axios.post("https://reeplay.online/location/loc", this.form);
